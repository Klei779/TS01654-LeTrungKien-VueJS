...
public class BillDAO extends CrudDAO<Bill, Long> {
    ...
    final String SELECT_BY_IDSTATUS_SQL = "SELECT * FROM Bills WHERE CardId=? AND Status=0";
    // SQL không có username
    final String findByTimeRange
            = "SELECT * FROM Bills WHERE Checkin BETWEEN ? AND ? ORDER BY Checkin DESC";
    // SQL có username
    final String findByUserAndTimeRange
            = "SELECT * FROM Bills WHERE username = ? AND Checkin BETWEEN ? AND ? ORDER BY Checkin DESC";
    ...
    public List<Bill> findByUserAndTimeRange(String username, Date begin, Date end) {
        return selectBySQL(findByUserAndTimeRange, username, begin, end);
    }

    public List<Bill> findByTimeRange(Date begin, Date end) {
        return selectBySQL(findByTimeRange, begin, end);
    }
...
// hàm lấy id có trạng thái status = 0 : chưa thanh toán

    public Bill selectByIDStatus0(Integer id) {
        List<Bill> list = selectBySQL(SELECT_BY_IDSTATUS_SQL, id);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
