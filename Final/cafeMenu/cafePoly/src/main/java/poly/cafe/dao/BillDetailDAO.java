...
public class BillDetailDAO extends CrudDAO<BillDetail, Long> {
    ...
    public List<BillDetail> selectBillId_lab4(long billid) {
        return selectBySQL(findByBillId_lab4, billid);
    }
    ...
    public void delete(Long billId, String drinkId) {
        String sql = "DELETE FROM BillDetails WHERE billid = ? AND drinkid = ?";
        XJDBC.update(sql, billId, drinkId);
    }

    public void deleteByBillId(Long billId) {
        String sql = "DELETE FROM BillDetails WHERE billid = ?";
        XJDBC.update(sql, billId);
    }
public BillDetail selectLast() {
    String sql = "SELECT TOP 1 * FROM Bills ORDER BY Id DESC";
    List<BillDetail> list = selectBySQL(sql);
